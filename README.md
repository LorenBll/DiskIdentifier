...
- **Persistence** — registered disk IDs are stored in the `DISK_IDENTIFIERS` variable in `.env`. Both the universal identifier and registered disk identifiers are automatically saved to `.env`.

> **Note:** The hashes stored by DiskIdentifier are plain identifiers used to associate disk roots with their registered IDs. They are **not** encryption keys. ServiceHandler's API key system (stored separately in `SH_API_KEYS`) is an independent mechanism — DiskIdentifier does not manage, encrypt, or store ServiceHandler API keys.
...
4. Review `resources/configuration.json` if you want to change the port.
...
DiskIdentifier does not manage, encrypt, or store API keys for ServiceHandler. Its identifiers are plain disk-root hashes used solely for disk registration and lookup. ServiceHandler's API key system is an entirely independent mechanism.
